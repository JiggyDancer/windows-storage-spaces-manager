...
class StorageApp(ctk.CTk):
    ...
    def setup_right_pane(self):
        ...
        ToolTip(self.vd_columns,
                "WARN: Must be <= number of disks in pool.\nMirror/Parity usually require even numbers.\nLeave blank for recommended value.")

        ctk.CTkLabel(new_frame, text="Interleave Size KB (Blank = Auto):").pack(anchor="w", padx=5, pady=(10, 0))
        self.vd_interleave = ctk.CTkEntry(new_frame, placeholder_text="256 KB (Default)")
    ...
    def _update_ui(self, disks, pools, topology):
        ...
        for pool_name, data in topology.items():
            pool_lbl = ctk.CTkLabel(self.topo_container, text=f"迪士 Pool: {pool_name}", font=("Arial", 14, "bold"),
                                    text_color="#3a7ebf")
    ...
    def validate_state(self, *args):
        ...
        valid_new_disks = [d for d in selected_disks if d.get("CanPool")]
        if len(valid_new_disks) > 0 and len(pool_name) > 0:
        ...
        if has_valid_pool and len(selected_disks) > 0:
            if any(d.get("CanPool") for d in selected_disks):
    ...
    def create_pool(self):
        ...
        try:
            disks = [cb.disk_obj for cb in self.disk_checkboxes if cb.get() == 1 and cb.disk_obj.get("CanPool")]
            storage_ops.create_pool(self.pool_name_var.get(), disks)
    ...
    def add_disks_to_pool(self):
        ...
        if not disks:
            messagebox.showerror("Error", "No eligible disks selected (Must be CanPool=True).")
            return
    ...
    def create_vd(self):
        ...
        if cols and not cols.isdigit():
            messagebox.showerror("Input Error", "Number of Columns must be an integer or blank.")
            return
